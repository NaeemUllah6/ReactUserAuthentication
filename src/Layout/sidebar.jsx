...
const menuItems = [
  ...
  { name: "Profile", path: "/profile", icon: <ClipboardDocumentListIcon className="w-5 h-5 mr-2" /> },
  { name: "People", path: "/people", icon: <BriefcaseIcon className="w-5 h-5 mr-2" /> },
  { name: "Explore Movies", path: "/movielist", icon: <CurrencyDollarIcon className="w-5 h-5 mr-2" /> },
  { name: "Add Movies", path: "/movieInput", icon: <BriefcaseIcon className="w-5 h-5 mr-2" /> },
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-screen  w-[200px] mb-[100px] p-1 bg-gray-900 text-white  transition-transform duration-300 ease-in-out z-30 
        ${offcanvas ? "translate-x-0" : "translate-x-[-100%] md:translate-x-0"}`}
      >
        <h2 className="font-bold text-2xl px-4">JimmyChoo</h2>
        <div className=" mt-10 overflow-auto h-[calc(100vh-100px)] pb-5">
